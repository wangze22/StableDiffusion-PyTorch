...
class Conv2d_lsq_144k(adda_cim.Conv2d_lsq_adda_cim):
    ...
    def cal_x_weight_block(self, x_expanded, w_qn):
        ...
        for key, weight_info in self.weight_mapping_info.items():
            ...
            # 逐 bit 计算
            # ===================== #
            output_bitwise, output_tar = self.cal_x_bitwise(x_split, w_qn_in)
            # scatter_plt(output_bitwise, output_tar,
            #             save_fig = 1,
            #             show_fig = 0,
            #             title = f'{self.name}_{key}',
            #             save_path = f'Scatter Plot')
            # ===================== #
            # 输出 bit 还原
            # ===================== #
            x = adda_cim.bit_concat_tensor(output_bitwise, data_bit = self.input_bit, slice_bit = self.dac_bit - 1)
            # ===================== #
            # 结果对应位置累加
            # ===================== #
    ...
    def cal_x_bitwise(self, x_expanded, w_qn):
        ...
        out_pos = self.sdk.calculate(to_numpy(x_expanded_pos),
                                     self.weight_addr,
                                     it_time = self.adc_gain.data.item())
        out_pos = to_tensor(out_pos, device = w_qn.device)

        out_neg = torch.matmul(x_expanded_neg, w_qn) * self.adc_scale
        out_neg = torch.clamp(out_neg, min = -self.adc_range - 1, max = self.adc_range)
        out_neg = torch.round(out_neg)

        out_array = out_pos + out_neg
        out_array = out_array.reshape(bit_len_batch, x_cols, -1)
        # ===================== #
        # 最终输出
        # ===================== #
        out = (out_array - out_adc_grad).detach() + out_adc_grad
        return out, out_adc_grad
    ...
    def forward(self, x):
        if self.use_FP():
            ...
            # 输入 bit 拆分
            # ===================== #
            x_expanded = adda_cim.bit_split_tensor(x_q = x_q,
...
class Linear_lsq_144k(adda_cim.Linear_lsq_adda_cim):
    ...
    def cal_x_bitwise(self, x_expanded, w_qn):
        ...
        # 144k片上计算
        # ===================== #
        x_expanded_np = to_numpy(x_expanded)
        x_expanded_np_pos = x_expanded_np + 0
        x_expanded_np_neg = x_expanded_np + 0
        x_expanded_np_pos[x_expanded_np_pos < 0] = 0
        x_expanded_np_neg[x_expanded_np_neg > 0] = 0
        out_pos = self.sdk.calculate(x_expanded_np_pos,
                                     self.addr, it_time = self.adc_gain)
        out_neg = np.matmul(x_expanded_np_neg, to_numpy(w_qn))
        out_array = out_pos + out_neg
