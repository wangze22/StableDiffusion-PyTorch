...
# ====================================================================== #
class Conv2d_lsq_adda_cim_multi_gains(nn.Conv2d):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
        ...
        for key, weight_info in self.weight_mapping_info.items():
            self.adc_gain_dict[key] = nn.Parameter(torch.tensor(self.adc_gain_min))
    ...
    def cal_x_weight_block(self, x_expanded, w_qn):
        ...
        for key, weight_info in self.weight_mapping_info.items():
            ...
            # 逐 bit 计算
            # ===================== #
            output_bitwise = self.cal_x_bitwise(x_split, w_qn_in, key)
            # ===================== #
            # 输出 bit 还原
            # ===================== #
            ...
            # 结果对应位置累加
            # ===================== #
            output_concat[:, :, start_col:start_col + col_num] += x / self.adc_scale_dict[key]
        return output_concat

    # @profile
    def cal_x_bitwise(self, x_expanded, w_qn, key):
        ...
        if self.adc_gain_dict[key] == self.adc_gain_min:
            adc_gain_ideal = init_adc_gain_(self, out_)
            if adc_gain_ideal is not None:
                self.adc_gain_dict[key].data = torch.tensor(adc_gain_ideal, device = self.adc_gain.device)
            else:
                pass
            adc_scale = get_adc_scale(self, self.adc_gain_dict[key])
            self.adc_scale_dict[key] = adc_scale
        out_adc = self.adc_scale_dict[key] * out_
    ...
    def refresh_adc_params(self):
        ...
        for key in self.adc_gain_dict.keys():
            self.adc_scale_dict[key] = get_adc_scale(self, self.adc_gain_dict[key])
...
class Linear_lsq_adda_cim_multi_gains(nn.Linear):
