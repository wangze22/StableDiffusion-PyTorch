...
# ====================================================================== #
class Conv2d_quant_noise_LSQ_bit_split(nn.Conv2d):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
    ...
    def forward(self, x):
        if self.use_FP():
            ...
            # 输入 bit 拆分
            # ===================== #
            x_list = bit_split(x_q = x, x_bit = self.input_bit, slice_bit = self.dac_bit - 1)
            # ===================== #
            # 权重截断&量化 + 噪声
            # ===================== #
            ...
            # 输出 bit 还原
            # ===================== #
            x = bit_concat(out_list, slice_bit = self.dac_bit - 1)
            # ===================== #
            # 量化系数还原
            # ===================== #
...
class Linear_quant_noise_LSQ_bit_split(nn.Linear):
...
def bit_split(x_q, x_bit, slice_bit):
    assert slice_bit >= 1
    bit_data_list = []
...
def bit_concat(bit_data_list, slice_bit):
