...
class Conv2d_lsq_int(nn.Module):
    ...
    def weight_quant_fine(self):
        ...
        w_data_int, _ = weight_quant_round(data_float = self.weight,
    ...
    def weight_quant_forward(self):
        w_step_size = self.lsq_quant_weight.step_size
        w_data_int, w_scale = weight_quant_floor(data_float = self.weight,
                                                 data_bit = self.weight_bit,
                                                 step_size = w_step_size, isint = 1)
    ...
    def forward(self, x):
        ...
        if self.int_grad:
            ...
            w_int, w_scale = self.weight_quant_forward()
        else:
