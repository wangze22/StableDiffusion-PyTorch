...
class Conv2d_lsq_int(nn.Module):
    def __init__(self, conv_lsq,
        ...
        self.step_size_input_init = step_size_input
        self.step_size_output_init = step_size_output
        self.step_size_weight_init = step_size_weight
        # 提取浮点数权重值
    ...
    def gen_ext_weight(self):
        ...
        w_data_int, w_scale = weight_quant_round(data_float = self.weight,
    ...
    def gen_ext_bias(self):
        self.lsq_quant_bias(self.bias)
        step_size_bias = self.lsq_quant_bias.step_size
        bias_data_int, bias_scale = weight_quant_round(data_float = self.bias,
                                                       data_bit = self.weight_bit,
                                                       step_size = step_size_bias,
                                                       isint = 0)

        bias_data_int = bias_data_int * self.y_shift_scale / self.step_size_input_init / self.step_size_weight_init * self.extended_levels
        bias_data_int = round_pass(bias_data_int)

        bias_ext_range = 2 ** (self.weight_bit + self.weight_bit_extension - 1) - 1
        bias_data_int = torch.clamp(bias_data_int, - bias_ext_range, bias_ext_range)

        self.bias_to_int_flag = True
    ...
    def bias_quant_fine(self):
        bias = floor_no_pass(self.bias.data)
        self.bias.data = bias

    def bias_quant_forward(self):
        bias = self.bias
        bias = self.grad_scale(bias, scale = self.extended_levels)
        bias_int = bias / self.extended_levels
        bias_int = floor_pass(bias_int)
        return bias_int
    ...
    def forward(self, x):
        ...
        if self.int_grad:
            y_shift = self.grad_scale(y_shift, 1 / y_shift_scale)
        # self.y_shift = y_shift
        # self.y_shift.retain_grad()
        self.y_shift_scale = y_shift_scale

        # MVM 结果加上 bias
        if self.bias is not None:
            if self.int_grad:
                if not self.bias_to_int_flag:
                    self.gen_ext_bias()
                self.bias_quant_fine()
                b_int = self.bias_quant_forward()
            else:
                b_int, b_scale = self.lsq_quant_bias(self.bias)
                b_int = round_pass(b_int * y_shift_scale * x_scale * w_scale)
                b_int = torch.clamp(b_int, -self.output_range, self.output_range)

            # self.b_int = b_int
            # self.b_int.retain_grad()

            y_shift = y_shift + b_int.view(1, -1, 1, 1)
