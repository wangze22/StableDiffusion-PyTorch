...
def collect_noise_folders(root: Path) -> List[Tuple[float, Path]]:
    """Find subdirectories under root whose names can be parsed as float.
    Returns a list of tuples (noise_value, path), sorted by noise_value asc.
    """
    folders: List[Tuple[float, Path]] = []
    for p in root.iterdir():
        if p.is_dir():
            val = is_float_name(p.name)
            if val is not None:
