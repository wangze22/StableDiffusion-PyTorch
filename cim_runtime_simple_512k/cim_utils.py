...
# MVM矩阵乘法计算函数
def mvm_calculate(chip_idx, input_matrix, addr, weight,
                  use_simulator = False, dac_bits = 4, adc_bits = 8,
                  noise_scale = 0.05):
    # chip_idx：芯片编号
    # input_matrix：输入矩阵
    # addr：权重地址
    ...
    if use_simulator:
        ...
        # MVM计算，加入了噪声模拟
        ADC_output = np.dot(input_expanded.transpose(1, 0), weight_n)
        # 仿真模拟计算的缩放效应
    ...
    # 使用忆阻器芯片计算
        ...
        row_block = 126
        weight_addr = addr
        start_row = weight_addr[0]
        start_col = weight_addr[1]
        row_num = weight_addr[2]
        col_num = weight_addr[3]
        ...
        ADC_output_total = None  # 用于累加每个块的输出
        while current_row < end_row:
            ...
            # 构造当前块的 weight_addr
            weight_addr_ = [current_row, block_row_num, start_col, col_num]

            # 调用 cima512k_batch_calc 进行计算
            _, ADC_output = op.cima512k_batch_calc(weight_addr_, input_expanded.transpose(1, 0), 2, 64)

            # 将结果累加
            if ADC_output_total is None:
                ADC_output_total = ADC_output
            else:
                ADC_output_total += ADC_output
